package com.example.project2.ui.movie_viewer;
...
import com.example.project2.ui.movie_list.Movies;
...
public class SlideshowFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        //Find ViewPager and TabLayout in the layout
        //ViewPager for swiping between movie details
        ViewPager2 viewPager = view.findViewById(R.id.view_pager);
        //TabLayout to display movie titles as tabs
        TabLayout tabLayout = view.findViewById(R.id.tabs);

        //Create a movie list and set up the adapter
        List<Movies> moviesList = createMovieList();
        //Adapter for the ViewPager
        MoviesPagerAdapter moviesPagerAdapter = new MoviesPagerAdapter(requireActivity(), moviesList);
        viewPager.setAdapter(moviesPagerAdapter);

        //Attach TabLayout with ViewPagerlg
        new TabLayoutMediator(tabLayout, viewPager,
                (tab, position) -> tab.setText(moviesList.get(position).getTitle())
        ).attach();

        return view;
    }
    //Create the list of movies again for the sake of simplicity in implementation
